...
calendar += calendar += '<h3>' + nextYear(y, m) + '年' + (nextMonth(y, m)+1) + '月' + '</h3>'
calendar += '<table>'
...
document.addEventListener('DOMContentLoaded', function(event){
        ...
        document.getElementById(`${i}`).addEventListener('click', 
            function(event){
                if (this.className == 'not_selected'){
                    ...
                    document.getElementById('id_days').value = JSON.stringify(days.sort((a,b) => a-b))
                }else{
    ...
    let formdays = document.getElementById('id_days').value
    formdays = JSON.parse(formdays)
